<template>
	<view class="vacation">
		...
		<view class="content">
			<u-swipe-action :show="item.show" :index="index" v-for="(item, index) in vacationList" :key="item.id" @open="open"
			 :options="options" @click="click">
		...
		<view class="plug">
			...
			<u-modal v-model="modalShow" :content="modalContent" :title="modalTitle" @confirm="modalConfirm" show-cancel-button></u-modal>
	...
	export default {
		...
		methods: {
			...
			sortVacList() {
				// 根据时间排序
				// 开始时间
				this.vacationList = this._.orderBy(this.vacationList, (o) => {
			...
			initVacList() {
				...
				// 防止读取不到数据
				if(!vacationDetail) return setTimeout(()=>{
					initVacList()
				},1000)
